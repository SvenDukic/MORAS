...
class Parser:
    ...
    from parseSymbs import _parse_symbols, _parse_labels, _parse_variables, _init_symbols, _unfold_macros
    ...
    def _iter_lines(self, func):
        ...
        for (line, p, o) in self._lines:
            ...
            if newline != "" and newline[0] == "*":
                ...
                x = l.split()
                for a in x:
                    newlines.append((a,i,o))
                    i += 1
            elif (len(newline) > 0):
                newlines.append((newline, i, o))
                i += 1
        self._lines = newlines
    def _blank(self, line, p, o):
        return line

    def _parse_whiles(self):
        newlines = []
        whilecnt = 0
        i = 0
        for (line, p, o) in self._lines:
            if line == "$END" and i != 0:
                ...
                while start >=0:
                    if newlines != []:
                        temp = newlines[start-1][0]
                    temp = temp.split("(")
                    if temp[0] == "$WHILE":
                        var = temp[1][0]
                        newlines[start-1] = ("*(WHILE" + str(whilecnt) + ") @" + var + " D=M @END" + str(whilecnt) + " D;JEQ", newlines[start-1][1], newlines[start-1][2])
                        break
                    start -= 1
                line = "*@WHILE" + str(whilecnt) + " 0;JMP (END" + str(whilecnt) + ")"
